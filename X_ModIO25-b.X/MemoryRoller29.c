...
//(5 para evitar que un registro invada otro en RAM)//
...
void     MrInicializa () {      // debe ser la primera rutina del Mem Roller
    unsigned short Ind;
...
void MrFormat () {      // Borra toda la memoria del Mem Roller
...
void MrInit () {
    ...
    // Solo sucederia si se agrega el ultimo registro y se va la luz.
    ...
    // Nunca deberia de suceder lo siguiente: Reg 0 es de fecha
...
void MrAgregaReg (){    // Si hay cambio de Fecha, agregar reg de fecha
    ...
    unsigned int MrTemp;
    //si no esta inicializado no hacer nada 
    RtcLee ();                          //Rtc = 1307
    MrSeparaFecha();                    // RtcFechaHora -> MrFecha12, MrHora12
...
unsigned int _MrSgte (unsigned int MrDirX){ // 0 -> (MrNRegMax-1) -> 0
    //si no esta inicializado no hacer nada 
    MrDirX ++;
...
#endif  // MrPrueba1
//------------------------------------------------------------------------------
#ifndef MrPrueba1   // No RAM. En memoria serial  
...
//    memcpy (&HiCars, &MrReg1[0], 4);    // Registra historial
//    HiRegistra (HiCars,MrRegPos);       // Registra historial
}   // fin de _MrEsc
#endif  // MrPrueba1
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
